...
%
% Last edited by Marie on 8/11/16. Based on K. Nagel's runNIWCwaveform.m
...
function [] = RunSingleTrial_DigitalAnalogOut_SaveData(TAKE_VIDEO, odor, concentration, stimulusFileName, expNumber)
AM_SYSTEMS = 0;
if nargin < 5
...
% if indicate 'random', randomly use one of a set of 10 pre-made random
%pseudo-random presentation sets!
if strcmp(stimulusFileName, 'random')
    randStimInd = randi(10);
    stimulusFileName = ['2016_11_30_controlStim_' num2str(randStimInd) '.mat']
end

load(['C:\Users\nagellab\Documents\Data\Stimuli\' stimulusFileName])
...
for trialNum = 1:length(valveOn)
    ...
    data(nn).scaleCurrent = 100;     % scaling factor for picoamps (with AM-Systems and 100x gain on Brownlee, OR Axon Instruments and 10x gain on Brownlee)
    data(nn).scaleVoltage = 10;      % scaling factor for mV (with 10x on Brownlee for both amps)
    ...
    addDigitalChannel(sd,'Dev1','Port0/Line6','OutputOnly');   %set up a fifth digital channel for camera trigger
    ...
    AOch = sa.addAnalogOutputChannel('Dev1',0,'Voltage'); %set up two channels for output (1 for valve control, one for video sync) 
    ...
    if TAKE_VIDEO > 0
        ...
        vid.FramesPerTrigger = data(nn).fps*data(nn).numSecOut; %matlab DOES NOT CARE what we enter here.
...
PlotTraces_singleFly(dateStr, expNumber, 0)
